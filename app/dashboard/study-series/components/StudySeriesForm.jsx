...
import {
  createStudySeries,
  validateSeriesCategory,
} from "@/app/actions/studySeries.action";
import { uploadFileToCloudinary } from "@/lib/upload";
import { studySeriesSchema } from "@/lib/validators/studySeries-schema";
...
const StudySeriesForm = ({ categories }) => {
  ...
  const {
    ...
    resolver: zodResolver(studySeriesSchema),
    defaultValues: {
      title: "",
      description: "",
      price: 0,
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const validation = studySeriesSchema.safeParse({
        ...data,
        outcomes,
        tags,
        thumbnail,
      ...
      const isValidcategory = await validateSeriesCategory(data?.category);
      if (!isValidcategory.success) {
        toast.error("Category does not exist!");
        return;
      }

      //uplood  thumbnail to cloudinary
      ...
      const res = await createStudySeries({
        ...data,
        thumbnailUrl,
        outcomes,
        tags,
      ...
      if (!res.success || !res?.seriesId) {
        toast.error(res?.message || "Failed to create series");
        return;
      }
  ...
  return (
    <form
      ...
      className="p-6 bg-white dark:bg-slate-950 rounded-lg shadow-md space-y-6"
    >
      {/* Basic Info */}
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
          ...
          <div>
            ...
            {errors.category?.part && (
              <p className="text-red-500 text-sm">
                {errors.category.part.message}
              </p>
            )}
          </div>
        </div>
