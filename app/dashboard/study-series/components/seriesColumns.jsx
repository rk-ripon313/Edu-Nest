  ...
  },
  {
    ...
    header: ({ column }) => (
      <button
        ...
        className="flex items-center justify-center gap-1 w-full"
      >
        Chapters <ArrowUpDown className="h-4 w-4" />
      </button>
    ),
    cell: ({ row }) => {
      const chapters = row.original.chapters || [];

      return <span className="text-center block">{chapters.length || 0}</span>;
    },
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-center justify-center gap-2">
        {/* Edit */}
        <Link href={`/dashboard/study-series/${row.original._id}/edit`}>
        ...
        {/* //other informations */}
        <Link href={`/dashboard/study-series/${row.original._id}/info`}>
          <Button variant="outline" size="icon" className="hover:scale-x-105">
