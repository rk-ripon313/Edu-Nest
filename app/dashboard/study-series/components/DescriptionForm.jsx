...
const SeriesDescriptionForm = ({ description = "", studySeriesId }) => {
  ...
  const onSubmit = async (values) => {
    try {
      ...
      if (result?.success) {
        ...
        router.refresh();
        toast.success(
          result.message || "Study-Series description has been updated."
        );
      } else {
      ...
      {isEditing && (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4">
          <Input
            ...
            placeholder="Enter Study-Series description"
          />
          {errors.description && (
...
export default SeriesDescriptionForm;
